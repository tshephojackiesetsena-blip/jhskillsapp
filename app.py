...
/* ── JH CoPilot ── */
.copilot-wrap{display:flex;flex-direction:column;height:calc(100vh - 120px);min-height:500px;background:var(--surface);border:1px solid var(--border);border-radius:var(--r);overflow:hidden}
.copilot-header{padding:16px 20px;border-bottom:1px solid var(--border);display:flex;align-items:center;gap:12px;background:var(--surface-2)}
.copilot-avatar{width:38px;height:38px;border-radius:10px;background:var(--jh-grad);display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0}
.copilot-title{font-family:'Syne',sans-serif;font-weight:700;font-size:15px;color:var(--text)}
.copilot-subtitle{font-size:11px;color:var(--text-3);margin-top:1px}
.copilot-status{width:8px;height:8px;border-radius:50%;background:#22c55e;margin-left:auto;flex-shrink:0;box-shadow:0 0 6px rgba(34,197,94,.5)}
.copilot-messages{flex:1;overflow-y:auto;padding:20px;display:flex;flex-direction:column;gap:14px}
.copilot-msg{display:flex;gap:10px;align-items:flex-start;max-width:80%;animation:msgIn .25s ease}
@keyframes msgIn{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}
.copilot-msg.user{align-self:flex-end;flex-direction:row-reverse}
.copilot-msg-avatar{width:30px;height:30px;border-radius:8px;flex-shrink:0;display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:700}
.copilot-msg.bot .copilot-msg-avatar{background:var(--jh-grad);color:#fff}
.copilot-msg.user .copilot-msg-avatar{background:var(--jh-teal);color:#fff}
.copilot-bubble{padding:11px 15px;border-radius:12px;font-size:13.5px;line-height:1.6;color:var(--text)}
.copilot-msg.bot .copilot-bubble{background:var(--surface-2);border:1px solid var(--border);border-top-left-radius:3px}
.copilot-msg.user .copilot-bubble{background:var(--jh-teal);color:#fff;border-top-right-radius:3px}
.copilot-bubble p{margin:0 0 8px}.copilot-bubble p:last-child{margin:0}
.copilot-bubble ul,.copilot-bubble ol{padding-left:18px;margin:6px 0}
.copilot-bubble li{margin-bottom:4px}
.copilot-bubble strong{font-weight:700}
.copilot-bubble code{background:rgba(0,0,0,.12);border-radius:4px;padding:1px 5px;font-size:12px;font-family:monospace}
.copilot-typing{display:flex;align-items:center;gap:5px;padding:11px 15px;background:var(--surface-2);border:1px solid var(--border);border-radius:12px;border-top-left-radius:3px;width:fit-content}
.copilot-typing span{width:7px;height:7px;border-radius:50%;background:var(--text-3);animation:typingDot 1.2s infinite}
...
.copilot-footer{padding:14px 16px;border-top:1px solid var(--border);background:var(--surface)}
.copilot-input-row{display:flex;gap:10px;align-items:flex-end}
.copilot-input{flex:1;resize:none;background:var(--bg-2);border:1.5px solid var(--border);border-radius:var(--r-sm);padding:10px 14px;color:var(--text);font-size:13.5px;font-family:'DM Sans',sans-serif;outline:none;line-height:1.5;max-height:120px;min-height:44px;transition:border-color var(--trans)}
.copilot-input:focus{border-color:var(--jh-teal);background:var(--surface)}
.copilot-send{width:42px;height:42px;border-radius:var(--r-sm);background:var(--jh-teal);border:none;color:#fff;cursor:pointer;display:flex;align-items:center;justify-content:center;flex-shrink:0;transition:all var(--trans);box-shadow:0 4px 12px rgba(0,168,157,.3)}
.copilot-send:hover{opacity:.88;box-shadow:0 6px 16px rgba(0,168,157,.45)}
.copilot-send:disabled{opacity:.45;cursor:not-allowed}
.copilot-chips{display:flex;flex-wrap:wrap;gap:7px;margin-bottom:12px}
.copilot-chip{font-size:12px;padding:5px 11px;border-radius:20px;border:1px solid var(--border);background:var(--surface-2);color:var(--text-2);cursor:pointer;transition:all var(--trans);font-family:'DM Sans',sans-serif}
.copilot-chip:hover{border-color:var(--jh-teal);color:var(--jh-teal);background:rgba(0,168,157,.06)}
.copilot-msg-time{font-size:10px;color:var(--text-3);margin-top:4px;text-align:right}
.copilot-msg.bot .copilot-msg-time{text-align:left}
.copilot-empty{flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:10px;opacity:.55;padding:40px}
.copilot-empty-icon{font-size:48px}
.copilot-empty-text{font-family:'Syne',sans-serif;font-size:16px;font-weight:700;color:var(--text)}
.copilot-empty-sub{font-size:13px;color:var(--text-2);text-align:center}
...
COPILOT_SYSTEM_PROMPT = """You are JH CoPilot, a friendly and knowledgeable AI assistant built into the JH Student Services portal. You help students and staff with:
- Academic questions (study tips, module concepts, exam preparation)
- Administrative queries (registration, fees, documents, bursaries)
- Campus life and services
- General learning and career guidance
- South African higher education context

Keep responses clear, concise, and supportive. Use bullet points and formatting where helpful. Always be encouraging and professional. If asked about something very specific (like a student's exact fee balance), let the user know they should check their portal or contact the relevant office directly."""


def _copilot_page(sidebar_fn, sidebar_path, back_url):
    student = current_student()
    admin = current_admin()
    user_name = (student["full_name"].split()[0] if student else (admin["name"].split()[0] if admin else "there"))

    content = f"""
<div class="page-header" style="margin-bottom:16px">
  <h1>🤖 JH CoPilot</h1>
  <p style="color:var(--text-2);font-size:13.5px;margin-top:4px">Your AI-powered study and admin assistant</p>
</div>

<div class="copilot-wrap" id="copilotWrap">
  <div class="copilot-header">
    <div class="copilot-avatar">🤖</div>
    <div>
      <div class="copilot-title">JH CoPilot</div>
      <div class="copilot-subtitle">Powered by Claude · JH Student Services</div>
    </div>
    <div class="copilot-status" title="Online"></div>
    <button onclick="clearChat()" class="btn btn-secondary btn-sm" style="margin-left:8px">Clear</button>
  </div>

  <div class="copilot-messages" id="copilotMessages">
    <div class="copilot-empty" id="copilotEmpty">
      <div class="copilot-empty-icon">🎓</div>
      <div class="copilot-empty-text">Hi {user_name}! I'm JH CoPilot.</div>
      <div class="copilot-empty-sub">Ask me anything about your studies, admin, fees, registration, or campus life.</div>
    </div>
  </div>

  <div class="copilot-footer">
    <div class="copilot-chips" id="copilotChips">
      <button class="copilot-chip" onclick="sendChip(this)">How do I register for modules?</button>
      <button class="copilot-chip" onclick="sendChip(this)">Study tips for exams</button>
      <button class="copilot-chip" onclick="sendChip(this)">What documents do I need?</button>
      <button class="copilot-chip" onclick="sendChip(this)">How does NSFAS work?</button>
      <button class="copilot-chip" onclick="sendChip(this)">Help me write a professional email</button>
    </div>
    <div class="copilot-input-row">
      <textarea class="copilot-input" id="copilotInput" placeholder="Ask JH CoPilot anything..." rows="1"
        onkeydown="handleKey(event)" oninput="autoResize(this)"></textarea>
      <button class="copilot-send" id="copilotSend" onclick="sendMessage()" title="Send">
...
function sendChip(btn){{
  document.getElementById('copilotInput').value = btn.textContent;
  document.getElementById('copilotChips').style.display = 'none';
  sendMessage();
}}
...
function clearChat(){{
  ...
  box.innerHTML = `<div class="copilot-empty" id="copilotEmpty">
    <div class="copilot-empty-icon">🎓</div>
    <div class="copilot-empty-text">Chat cleared!</div>
    <div class="copilot-empty-sub">Start a new conversation anytime.</div>
  </div>`;
  document.getElementById('copilotChips').style.display = 'flex';
}}
...
function appendMsg(role, text){{
  const empty = document.getElementById('copilotEmpty');
  if(empty) empty.remove();
  const box = document.getElementById('copilotMessages');
  const isUser = role === 'user';
  const initials = isUser ? '{user_name[0].upper()}' : '🤖';
  const div = document.createElement('div');
  div.className = 'copilot-msg ' + role;
  div.innerHTML = `
    <div class="copilot-msg-avatar">${{isUser ? initials : '🤖'}}</div>
    <div>
      <div class="copilot-bubble" id="bubble-${{Date.now()}}">${{isUser ? escHtml(text) : marked(text)}}</div>
      <div class="copilot-msg-time">${{nowTime()}}</div>
    </div>`;
  box.appendChild(div);
...
function showTyping(){{
  ...
  div.innerHTML = `<div class="copilot-msg-avatar">🤖</div>
...
function marked(text){{
  return text
    .replace(/```([\\s\\S]*?)```/g, '<pre style="background:rgba(0,0,0,.1);border-radius:6px;padding:10px;overflow-x:auto;font-size:12px;font-family:monospace;margin:6px 0"><code>$1</code></pre>')
    .replace(/`([^`]+)`/g, '<code>$1</code>')
    .replace(/\\*\\*(.+?)\\*\\*/g, '<strong>$1</strong>')
    .replace(/\\*(.+?)\\*/g, '<em>$1</em>')
    .replace(/^### (.+)$/gm, '<h4 style="font-family:Syne,sans-serif;margin:8px 0 4px">$1</h4>')
    .replace(/^## (.+)$/gm, '<h3 style="font-family:Syne,sans-serif;margin:8px 0 4px">$1</h3>')
    .replace(/^# (.+)$/gm, '<h2 style="font-family:Syne,sans-serif;margin:8px 0 4px">$1</h2>')
    .replace(/^\\* (.+)$/gm, '<li>$1</li>')
    .replace(/^- (.+)$/gm, '<li>$1</li>')
...
async function sendMessage(){{
  ...
  try {{
    ...
    if(data.ok){{
      ...
      appendMsg('bot', '⚠️ ' + (data.error || 'Something went wrong. Please try again.'));
    }}
  }} catch(e) {{
    removeTyping();
    appendMsg('bot', '⚠️ Could not reach JH CoPilot. Please check your connection.');
  }}
...
"""
    return render_shell(content, "JH CoPilot", sidebar_fn(sidebar_path), "JH CoPilot")


@app.route("/student/copilot")
@student_required
def student_copilot():
    return _copilot_page(student_sidebar, "/student/copilot", "/student/dashboard")


@app.route("/admin/copilot")
@admin_required
def admin_copilot():
    return _copilot_page(admin_sidebar, "/admin/copilot", "/admin/dashboard")
...
def copilot_chat():
    ...
    if not current_student() and not current_admin():
    ...
    if not api_key:
        return jsonify({"ok": False, "error": "AI service not configured. Please set ANTHROPIC_API_KEY."}), 500
    ...
    try:
        ...
        return jsonify({"ok": False, "error": f"AI error: {e.code}"}), 500
    except Exception as e:
        return jsonify({"ok": False, "error": "Failed to reach AI service."}), 500
